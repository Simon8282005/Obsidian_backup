...
`print(3 / 2` 两个号码都是属于整数，电脑也自动的返回整数给我们了，`1.5` 电脑直接吧 `0.5` 丢掉，放回 `1` 给我们
...
##### 数据类型是很重要的！！！！千万要精通啊！！！要不然会出现一大堆的 bug

---
# Lesson 28: Specifying types with type hints

因为 GDScript 是一个动态语言，就像 Python，不一定要在变量名前面添加关键字，只需要直接给变量一个数值就行，但这也导致了一个很大的问题，这些错误并不会直接出现，而是当我们调用了包含这些错误的函数才会出现，这就导致后期需要更大的成本来排查错误的根源，而且随着代码量的提升，找到错误的成功率和需要的时间也会逐渐增加

但不用紧，我们可以用 Type hints 来解决这个问题

Type hints 告诉电脑这个变量只能接受哪一种类型的数据，这样我们就能尽早的知道我们是不是搞错数据的类型了

GDScript 里的 Type Hints 是这样的: 

![[Pasted image 20230402104255.png]]

如果不想写那么长的话，我们也能这样写，这是让电脑制自动帮我们定义该变量的数据类型：

![[Pasted image 20230402104446.png]]

---

好了，经过七七四十九天的努力，成功把 28 堂 Godot 基础班学完了 （完结撒花 ✿✿ヽ(°▽°)ノ✿）
